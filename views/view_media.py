...
class MediaGalleryView(ctk.CTkFrame):
    ...
    def update_measurement_list(self):
        ...
        for i, m in enumerate(reversed(measurements)):
            btn = ctk.CTkButton(self.scroll_list, 
                                text=f"Ciclo #{m.get('cycle_id', len(measurements)-i)}\n{m.get('model', 'N/A')}", 
    ...
    def show_details(self, measurement):
        ...
        ctk.CTkLabel(header, text=f"Detalle Visual: Ciclo #{measurement.get('cycle_id')}", 
                     font=ctk.CTkFont(size=22, weight="bold")).pack(side="left")
        
        ctk.CTkLabel(header, text=f"Total: {measurement.get('total_time')}s", 
