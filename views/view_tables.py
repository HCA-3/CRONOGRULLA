...
class TablesView(ctk.CTkFrame):
    ...
    def update_table_view(self, model_name):
        ...
        self.main_title.configure(text=f"{model_name} - Matriz de Tiempos")
    ...
    def delete_last_measurement(self):
        ...
        if messagebox.askyesno("Confirmar", "¿Mover el último ciclo a la papelera?"):
            if "trash" not in self.app.data:
                self.app.data["trash"] = []
            
            # Mover a papelera
            item = measurements.pop()
            self.app.data["trash"].append(item)
            
            self.app.save_data()
            self.update_table_view(self.table_selector_var.get())
            messagebox.showinfo("CronoGrulla", "El registro se ha movido a la papelera.")
